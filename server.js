...
// LOGIC TRÍCH XUẤT DỮ LIỆU GAME (MỚI - BẢO MẬT)
...
// FULL AI LOGIC (GIỮ NGUYÊN CỦA DŨNG)
...
// ENDPOINT DỰ ĐOÁN (MỚI - BẢO MẬT TUYỆT ĐỐI)

// ==========================================

app.post("/get-prediction-v2", async (req, res) => {
...
async function loadValidKeys() {
    ...
    return keys.filter(k => !k.expire || k.expire > now);
...
app.post("/check-key", async (req, res) => {
    ...
    if (found.game && game && found.game !== game) return res.json({ success: false, message: "Sai game" });
...
// ADMIN PANEL HTML (BẢN GỐC CỦA DŨNG)
